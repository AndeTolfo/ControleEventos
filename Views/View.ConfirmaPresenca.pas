...
uses DmModule, View.Presencas;
  ...
  var reimprimircracha, Resposta : integer;
  begin
      // verificar se a inscrição já tem presença registrada, se sim imprimir somente o cracha
      DM.QueryPresencas.SQL.Clear;
      DM.QueryPresencas.SQL.Add('select p.cod_inscricao from presencas p where p.cod_inscricao = :cod');
      DM.QueryPresencas.ParamByName('cod').AsInteger := CodInscricao;
      DM.QueryPresencas.Open;
      reimprimircracha := DM.QueryPresencas.FieldByName('cod_inscricao').AsInteger;
      if reimprimircracha > 0 then
      begin
        Resposta := MessageDlg('Presença já está registrada, deseja reimprimir o crachá?', mtConfirmation, [mbYes, mbNo], 0);
      ...
      begin
    if StrToDate(data_ev)< Now then
    begin
      ShowMessage('A data do evento já passou. Não é possível confirmar a presença.');
      Exit;
    end;
         // INSERT PRESENÇAS
     DM.QueryPresencas.SQL.Clear;
     ...
     // Define cod inscrição para impressão do cracha
     // Exiba uma mensagem informando que a inscrição foi realizada com sucesso
      ShowMessage('Presença Confirmada');
      QueryEtiqueta.ParamByName('cod').AsInteger := CodInscricao;
      QueryEtiqueta.Open;
      ppReport.Print;
