...
begin
     if not Funcoes.VerificarCamposVazios([edtNomeFPG]) then
     begin
...
//    frmFormasPag.Height := 422;
  if DM.QueryFormasPag.State in [dsEdit, dsInsert] then
  begin
    ...
    case ConfirmaAlteracoes of
      ...
      if not Funcoes.VerificarCamposVazios([edtNomeFPG]) then
        begin
        ShowMessage('Valores incorretos, alteração cancelada');
        end
        else
        begin
        DM.QueryFormasPag.Post;
